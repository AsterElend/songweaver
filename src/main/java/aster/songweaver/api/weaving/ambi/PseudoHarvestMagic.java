package aster.songweaver.api.weaving.ambi;
...
public class PseudoHarvestMagic implements Draft, Ritual {
    ...
    public void cast(ServerWorld world, ServerPlayerEntity caster, @Nullable JsonObject data) {
        ...
        if (!allowedTargets.contains(target.getType()) || SongweaverComponents.SILENCE.get(target).isSilenced()){
    ...
    @Override
    public void ritualCast(ServerWorld world,
                           ServerPlayerEntity caster, GrandLoomBlockEntity loom, @Nullable JsonObject data) {
      ...
      if (data == null){
        ...
        for (LivingEntity target : targets) {
            if (allowedTargets.contains(target.getType())
                    && !SongweaverComponents.SILENCE.get(target).isSilenced()) {
   ...
   public static void finalExecution(ServerWorld world, LivingEntity target, @Nullable JsonObject data, ServerPlayerEntity caster){
       ...
       SongweaverComponents.SILENCE.get(target).setSilence(silenceTicks);
